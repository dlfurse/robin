...
namespace robin
{
    ...
    {
        thread* t_thread = reinterpret_cast< ::robin::thread* >( t_thread_ptr );
        t_thread->f_state_mutex.lock();
        t_thread->f_state = ::robin::thread::e_executing;
        t_thread->f_state_mutex.unlock();
        t_thread->f_thread_mutex.unlock();
        if( t_thread->f_start != NULL )
        {
            pthread_cleanup_push( &::robin::thread::thread_stop, t_thread_ptr );
            t_thread->f_start->execute();
            pthread_cleanup_pop( 0 );
        }
        t_thread->f_state_mutex.lock();
        t_thread->f_state = ::robin::thread::e_executed;
        t_thread->f_state_mutex.unlock();
        return t_thread_ptr;
    }
    ...
    void thread::thread_stop( void* t_thread_ptr )
    {
        thread* t_thread = reinterpret_cast< ::robin::thread* >( t_thread_ptr );
        t_thread->f_state_mutex.lock();
        t_thread->f_state = ::robin::thread::e_cancelling;
        t_thread->f_state_mutex.unlock();
        t_thread->f_thread_mutex.unlock();
        ...
        t_thread->f_state_mutex.lock();
        t_thread->f_state = ::robin::thread::e_cancelled;
        t_thread->f_state_mutex.unlock();
        return;
    }
